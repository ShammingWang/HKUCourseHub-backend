...
from backend.app.main.schema.token import GetLoginToken, GetNewToken

from backend.app.main.model import User
from backend.app.main.crud.crud_user import user_dao
from backend.app.main.schema.user import AuthLoginParam
...
class AuthService:
    ...
    async def swagger_login(self, *, obj: HTTPBasicCredentials) -> tuple[str, User]:
        async with async_db_session.begin() as db:
            ...
            if user:
                await db.refresh(user)  #让 user 重新绑定到当前事务
    ...
    async def login(
        ...
        async with async_db_session.begin() as db:
            ...
            try:
                ...
                #     ),
                # )
                raise errors.AuthorizationError(msg=e.msg, background=None)
            except Exception as e:
                log.error(f'登陆错误: {e}')
                raise e
            # try代码块没有异常则执行else代码块
            else:
    ...
    async def logout(*, request: Request, response: Response) -> None:
        ...
        # 强制不允许多点登录
        is_multi_login = False
        if is_multi_login: # if request.user.is_multi_login:
